...
from utils import open_sync


class DataStoreCRD:
    def check_time_to_live(self, value):
        # How long is the data accessible.

        created_time = value['CreatedAt']

        # Parsing of the datetime from the string date.
        created_time = parse(created_time)

        time_to_live = value['Time-To-Live']

        if time_to_live is not None:
            # Calculating the data expire time.
            expired_datetime = created_time + timedelta(seconds=time_to_live)

            # Calculating the remaining seconds of expired time(may/may not expired) from current time.
    ...
    def check_create_data(self, json_data, db_path):
        ...
        #  If size is greater than 1GB ignore the data.
        ...
        # Write the new data.
        with open_sync(datastore, 'w') as f:
            json.dump(data, f)
